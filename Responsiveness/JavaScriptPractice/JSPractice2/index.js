...
allProductLists.map(function (singleProduct, index) {
  ...
  <div class="productDesc">
    ...
    <div class="priveCart">
      ...
      <button data-productIndex="${singleProduct.id}" class="addCart">
        <svg
          width="35"
          height="34"
          viewBox="0 0 35 34"
      ...
      <i class="bx bxs-trash removeCart hideTag" data-productId = "${singleProduct.id}"></i>
    </div>
  </div>
</div>`;
});
...
// select all products cart buttons
const allProductsRemoveFromCart = document.getElementsByClassName("removeCart");

//get all cart products from local storage
const getCart = () => JSON.parse(localStorage.getItem("myCart")) || [];

//create cart local storage
const setCart = (productId, cartType = "add") => {
  let collectCart = getCart();

  //declaring a variable called newCart without assigning any value to it
  let newCart;
  //check if cart type is to add or remove product from the cart
  if (cartType == "add") {
    //selecting one product from the database by the index number
    newCart = allProductLists.filter(
      (filterData) => filterData.id == productId
    );
    collectCart.push(newCart);
  } else {
    newCart = collectCart.filter((filterData) => filterData.id != productId);
    console.log('before', collectCart)
    let collectCartD = collectCart.findIndex((filterData) => filterData.id != productId);
    collectCart.splice(collectCartD, 1);
    console.log('index to be removed', typeof productId)
    console.log('after', collectCart)
  }

  //add the new product to the existing cart array list


  //set the collated cart list to the local storage
  localStorage.setItem("myCart", JSON.stringify(collectCart));
};
...
for (let i = 0; i < allProductsAddToCart.length; i++) {
  allProductsAddToCart[i].addEventListener("click", function () {
    ...
    //Display the counter
    counterTag.textContent = getCartCounter();

    //Hide the add to cart button for the added product
    allProductsAddToCart[i].classList.add("hideTag");
    allProductsRemoveFromCart[i].classList.remove("hideTag");

    //get productId number of the selected product
    let productId = allProductsAddToCart[i].getAttribute("data-productId");
    setCart(index);
  });

  //Remove or reverse clicked Cart
  allProductsRemoveFromCart[i].addEventListener("click", function () {
    //get current Cart counter from local storage
    let getCartCount = getCartCounter();

    //deducting cart counter
    setCartCounter(parseInt(getCartCount) - 1);

    //Hide the add to cart button for the added product
    allProductsAddToCart[i].classList.remove("hideTag");
    allProductsRemoveFromCart[i].classList.add("hideTag");

    //Display the counter
    counterTag.textContent = getCartCounter();

    let productId = allProductsRemoveFromCart[i].getAttribute("data-productId");
    setCart(productId, "removed");
  });
}
